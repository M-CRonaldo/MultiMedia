...
// Value update using spin control
function spinControl(e) {
    var target = $(e.target);
    if (!$(target).data("delta")) {
        target = $(e.target).closest("button");
    }
    var delta = parseInt(target.data("delta"));

    var controlToSpin = $("#" + $(target).data("spinTarget"));
    var newValue = parseInt(controlToSpin.val()) + delta;
    var minValue = controlToSpin.attr("min");
    var maxValue = controlToSpin.attr("max");
    if (newValue >= minValue && newValue <= maxValue) {
        controlToSpin.val(newValue).change();
    }
}
...
$(document).ready(function() {
    ...
    $(".spin-control").on("click", spinControl); // Spin-able is clicked
    ...
    $("#karplus-base, input").not("[id^=import], #zoomStartFrom").on("change", updateWaveformDisplay).first().change();
